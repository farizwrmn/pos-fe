  ...
  effects: {
    ...
    *queryPosDetail({ payload }, { call, put }) {
      ...
      if (data && PosData && PosData.success) {
        ...
        for (let n = 0; n < data.pos.length; n += 1) {
          if (data.pos[n].typeCode === "P") {
            ...
            dataPos.push({
              ...
              total:
                data.pos[n].qty * data.pos[n].sellingPrice -
                data.pos[n].discount * data.pos[n].qty -
                data.pos[n].qty *
                  data.pos[n].sellingPrice *
                  (data.pos[n].disc1 / 100) -
                data.pos[n].qty *
                  data.pos[n].sellingPrice *
                  (data.pos[n].disc1 / 100) *
                  (data.pos[n].disc2 / 100) -
                data.pos[n].qty *
                  data.pos[n].sellingPrice *
                  (data.pos[n].disc1 / 100) *
                  (data.pos[n].disc2 / 100) *
                  (data.pos[n].disc3 / 100),
            });
        ...
        // service charge gadipakai
        let serviceChargePrint = [];
        if (PosData.pos.dineInTax) {
          serviceChargePrint = serviceChargePrint.concat([
            {
              alignment: "two",
              text: `Service Charge:Rp`,
              rightText: `${numberFormatter(PosData.pos.dineInTax)}`,
            },
          ]);
        }
        ...
        let TotalConsignment = invoiceInfo.unitInfo.dataConsignment.reduce(
          (cnt, o) => cnt + o.total,
          0
        );
        const cash =
