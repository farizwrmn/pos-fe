...
import { query, add, edit, remove } from '../../services/contractType'
import { pageModel } from '../common'

const success = () => {
  message.success('Contract Type has been saved')
}
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        ...
        if (pathname === '/setting/store' || pathname === '/master/employee' || pathname === '/contract-type') dispatch({ type: 'query' })
        else if (pathname === '/master/contract-type') {
        ...
        } else if (pathname === '/master/customer' && activeKey === '2') dispatch({ type: 'contractType/query' })
      })
    }
  },
  ...
  effects: {
    ...
    * delete ({ payload }, { call, put, select }) {
      ...
      const { selectedRowKeys } = yield select(models => models.contractType)
      if (data.success) {
