...
const ModalEntry = ({
  ...
  const optionRole = listRole && listRole.length > 0
    ? listRole.filter(filtered => filtered.key !== 'OWN' && filtered.key !== 'SPR').map(c => <Option value={c.key} disabled={!(listUserRole.includes(c.key))}>{c.title}</Option>)
    : []
    ...
    >
      <Tabs type="card" activeKey={activeTab} size="small" tabPosition="top" onChange={hdlTabCallback}>
        ...
        <TabPane tab="Role" key="3">
          <Transfer
            dataSource={listRole && listRole.filter(filtered => filtered.key !== 'OWN' && filtered.key !== 'SPR')}
