  ...
  const createTableBody = (tabledata) => {
    ...
    for (let key in rows) {
      if (rows.hasOwnProperty(key)) {
        ...
        row.push({ text: formatNumberIndonesia(data.adjInQty > 0 ? data.adjInQty * data.sellingPrice : data.adjOutQty * data.sellingPrice || 0), alignment: 'right', fontSize: 11 })
        body.push(row)
      }
      count += 1
    }
    ...
    stack: [
      ...
      },
      {
        table: {
          ...
          body: [
            [{ text: 'NO TRANSAKSI', fontSize: 11 }, ':', { text: (itemHeader.transNo || '').toString(), fontSize: 11 }, {}, { text: 'REF DATE', fontSize: 11 }, ':', { text: moment(itemHeader.referenceDate).format('DD-MM-YYYY'), fontSize: 11 }],
