  ...
  const createTableBody = (tableBody) => {
    ...
    for (let key in tableBody) {
      if (tableBody.hasOwnProperty(key)) {
        ...
        row.push({ text: tableBody[key].transNo || '', alignment: 'left' })
        row.push({ text: currencyFormatter(tableBody[key].debit || 0), alignment: 'right' })
        row.push({ text: currencyFormatter(tableBody[key].credit || 0), alignment: 'right' })
        row.push({ text: tableBody[key].description, alignment: 'right' })
        row.push({ text: tableBody[key].recon === 1 ? 'Recon' : 'Not Recon', alignment: 'right' })
        body.push(row)
      }
      count += 1
    }
