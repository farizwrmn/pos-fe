...
const Detail = ({ bankentry, app, dispatch }) => {
  const { listDetail, data } = bankentry
  ...
  const BackToList = () => {
    dispatch(routerRedux.push('/bank-entry?activeKey=1'))
  }
...
export default connect(({ app, bankentry }) => ({ app, bankentry }))(Detail)
