  ...
  const onPrintPDF = () => {
    function createPdfLineItems(tabledata) {
      var headers = {
        top:{
          ...
          col_2:{ text: 'Nama Brand', style: 'tableHeader', alignment: 'center', bold: true, fontSize: 13, style: 'headers'},
          col_3:{ text: 'Gambar', style: 'tableHeader', alignment: 'center', bold: true, fontSize: 13, style: 'headers'},
      ...
      for (var key in rows)
      {
        if (rows.hasOwnProperty(key))
        {
          ...
          var data = rows[key];
          var row = new Array();
          row.push( { text: (data.brandCode||'').toString(), alignment: 'center'  } )
          row.push( { text: (data.brandName||'').toString(), alignment: 'center' } )
          row.push( { text: (data.brandImage||'').toString(), alignment: 'center' } )
          body.push(row);
        }
    ...
    var docDefinition = {
      ...
      content: [
        ...
        { text: 'LAPORAN DAFTAR BRAND', margin: [210, 10], style: 'header' },
        {
          style: 'tableExample',
          writable: true,
          table: {
