...
import { Form, Modal, AutoComplete, Input, InputNumber, message, DatePicker, Select } from 'antd'
...
class ModalSticker extends Component {
  ...
  render () {
    ...
    const modalProps = {
      ...
      title: !update ? (modalProductType === 'all' || modalProductType === 'consignment' ? 'All Product' : 'Update Product') : 'Edit Product',
      width: 400,
      onOk () {
        const data = {
          ...getFieldsValue()
    ...
    } else if (modalProductType === 'consignment') {
      fieldName = (<Input {...inputProps} />)
    ...
    return (
      <Modal {...modalProps}>
        ...
        <FormItem label="Qty" >
          {getFieldDecorator('qty', { initialValue: selectedSticker.qty || 1 })(<InputNumber {...inputNumberProps} />)}
        </FormItem>
      </Modal>
    )
  }
