  ...
  const createPdfLineItems = (tabledata) => {
    let headers = {
      top:{
        ...
        col_2: { text: 'NAMA KATEGORI', style: 'tableHeader', alignment: 'center', bold: true, fontSize: 13 },
      },
    }
    let rows = tabledata
    let body = []
    ...
    for (var key in rows)
    {
      if (rows.hasOwnProperty(key))
      {
        var data = rows[key]
        var row = new Array()
        row.push({ text: (data.categoryCode || '').toString(), alignment: 'left' })
        row.push({ text: (data.categoryName || '').toString(), alignment: 'left' })
        body.push(row)
      }
    }
    return body
  ...
  const handlePDF = (dataSource) => {
    ...
    let docDefinition = {
      ...
      header: [
        ...
        {text: 'LAPORAN DAFTAR KATEGORI BARANG', fontSize: 16, margin: [45, 2, 0, 0]},
...
export default connect(({ app, category }) => ({ app, category }))(PrintPDF)
