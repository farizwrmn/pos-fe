...
const PrintPDF = ({ dataSource, user, storeInfo }) => {
  let tableHeaders = {
    top: {
      col_1: { text: 'CATEGORY CODE', style: 'tableHeader', alignment: 'center', bold: true, fontSize: 13, style: 'headers' },
      col_2: { text: 'CATEGORY NAME', style: 'tableHeader', alignment: 'center', bold: true, fontSize: 13, style: 'headers' },
    },
  }
  ...
  const createTableBody = (tableBody) => {
    ...
    for (let key in tableBody) {
      if (tableBody.hasOwnProperty(key)) {
        let row = []
        row.push({ text: (tableBody[key].categoryCode || '').toString(), alignment: 'left' })
  ...
  const header = [
    ...
    { text: 'LAPORAN DAFTAR KATEGORI BARANG', style: 'headerTitle' },
  ]
