...
const PrintShelf = ({ dataSource, user, storeInfo }) => {
  const createTableBody = (tableBody) => {
    ...
    for (let key in tableBody) {
      if (tableBody.hasOwnProperty(key)) {
        ...
        row.push({
          columns: [
            { text: `Rp ${(tableBody[key].sellPrice || 0).toLocaleString()}`, style: 'sellPrice' },
            { text: '(Sellprice)', style: 'info', margin: [0, 12, 0, 0] }
          ]
        })
        row.push({
          columns: [
            { text: `Rp ${(tableBody[key].distPrice01 || 0).toLocaleString()}`, style: 'others' },
            { text: '(Dist price 01)', style: 'info', margin: [0, 5, 0, 0] }
          ]
        })
        row.push({
          columns: [
            { text: `Rp ${(tableBody[key].distPrice02 || 0).toLocaleString()}`, style: 'others' },
