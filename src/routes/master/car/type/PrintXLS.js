...
const PrintXLS = ({ data, storeInfo, name }) => {
  ...
  const createTableBody = (list) => {
    ...
    for (let key in list) {
      if (list.hasOwnProperty(key)) {
        ...
        row.push({ value: (list[key].typeName || '').toString(), alignment: { vertical: 'middle', horizontal: 'left' }, font: styles.tableBody, border: styles.tableBorder })
        body.push(row)
      }
      start += 1
    }
  ...
  const title = [
    { value: 'LAPORAN DAFTAR TIPE', alignment: { vertical: 'middle', horizontal: 'center' }, font: styles.title },
