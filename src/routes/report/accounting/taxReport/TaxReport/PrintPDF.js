...
const PrintPDF = ({ user, listTrans, storeInfo, fromDate, toDate }) => {
  ...
  const createTableBody = (tabledata) => {
    ...
    for (let key in rows) {
      if (rows.hasOwnProperty(key)) {
        ...
        row.push({ text: (data.reference || '').toString(), alignment: 'left', fontSize: 11 })
        row.push({ text: moment(data.transDate).format('DD-MMM-YYYY'), alignment: 'left', fontSize: 11 })
        row.push({ text: (data.vendor || '').toString(), alignment: 'left', fontSize: 11 })
        row.push({ text: (data.buktiBayar || '').toString(), alignment: 'left', fontSize: 11 })
  ...
  const tableHeader = [
    [
      ...
      { fontSize: 12, text: 'VENDOR', style: 'tableHeader', alignment: 'center' },
      { fontSize: 12, text: 'BUKTI BAYAR', style: 'tableHeader', alignment: 'center' },
