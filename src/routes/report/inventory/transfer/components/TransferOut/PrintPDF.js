    ...
    for (let key in rows) {
      if (rows.hasOwnProperty(key)) {
        ...
        let row = [
          ...
          { text: formatNumberIndonesia(data.netto || 0), alignment: 'right', fontSize: 11 },
  ...
  for (let key in listData) {
    ...
    tableTitle.push({
      table: {
        ...
        body: [
          [{ text: 'NO TRANSAKSI' }, ':', { text: listData[key][0].transNo }, {}, { text: 'TANGGAL' }, ':', { text: moment(listData[key][0].transDate).format('DD-MMM-YYYY') }]
        ]
      },
      style: 'tableTitle',
      layout: 'noBorders'
