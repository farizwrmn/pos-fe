  ...
  const handleXLS = () => {
    if (period === '' && year === '') {
      ...
      for (let n = 0; n < listRekap.length; n += 1) {
        ...
        sheet.getCell(`K${m}`).value = `${(parseFloat(listRekap[n].posQty))}`
        sheet.getCell(`K${m}`).alignment = { vertical: 'middle', horizontal: 'right' }
        sheet.getCell(`L${m}`).value = `${(parseFloat(listRekap[n].posPrice))}`
        sheet.getCell(`L${m}`).alignment = { vertical: 'middle', horizontal: 'right' }
        sheet.getCell(`M${m}`).value = `${(parseFloat(listRekap[n].adjOutQty))}`
        sheet.getCell(`M${m}`).alignment = { vertical: 'middle', horizontal: 'right' }
        sheet.getCell(`N${m}`).value = `${(parseFloat(listRekap[n].adjOutPrice))}`
        sheet.getCell(`N${m}`).alignment = { vertical: 'middle', horizontal: 'right' }
        sheet.getCell(`O${m}`).value = `${(parseFloat(listRekap[n].count))}`
        sheet.getCell(`O${m}`).alignment = { vertical: 'middle', horizontal: 'right' }
        sheet.getCell(`P${m}`).value = `${(parseFloat(listRekap[n].amount))}`
