  ...
  const handleXLS = () => {
    if (fromDate === '' && toDate === '') {
      ...
      for (let n = 0; n < listTrans.length; n += 1) {
        ...
        sheet.getCell(`G${m}`).numFmt = formatNumberInExcel((parseFloat(listTrans[n].service) + parseFloat(listTrans[n].product) + parseFloat(listTrans[n].counter)), 2)
      }
