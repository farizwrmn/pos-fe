...
const PrintXLS = ({ listTrans, fromDate, toDate, storeInfo }) => {
    ...
    for (let key in rows) {
      if (rows.hasOwnProperty(key)) {
        ...
        let row = [
          ...
          { value: data.policeNo, alignment: { vertical: 'middle', horizontal: 'left' }, font: styles.tableBody, border: styles.tableBorder },
          { value: parseFloat(data.product || 0), alignment: { vertical: 'middle', horizontal: 'right' }, font: styles.tableBody, border: styles.tableBorder },
          { value: parseFloat(data.service || 0), alignment: { vertical: 'middle', horizontal: 'right' }, font: styles.tableBody, border: styles.tableBorder },
          { value: parseFloat(data.product || 0) + parseFloat(data.service || 0), alignment: { vertical: 'middle', horizontal: 'right' }, font: styles.tableBody, border: styles.tableBorder }
        ]
        body.push(row)
      }
      start += 1
  ...
  const title = [
    { value: 'LAPORAN JASA + PRODUCT PER FAKTUR', alignment: { vertical: 'middle', horizontal: 'center' }, font: styles.title },
  ...
  const tableHeader = [
    [
      ...
      { value: 'MEMBER', alignment: { vertical: 'middle', horizontal: 'center' }, font: styles.tableHeader, border: styles.tableBorder },
  ...
  const tableFooter = [
    [
      ...
      { value: productTotal, alignment: { vertical: 'middle', horizontal: 'center' }, font: styles.tableFooter, border: styles.tableBorder },
      { value: serviceTotal, alignment: { vertical: 'middle', horizontal: 'center' }, font: styles.tableFooter, border: styles.tableBorder },
      { value: parseFloat(serviceTotal) + parseFloat(productTotal), alignment: { vertical: 'middle', horizontal: 'center' }, font: styles.tableFooter, border: styles.tableBorder }
    ]
  ]
