...
const PrintXLS = ({ listTrans, dataSource, fromDate, toDate, storeInfo }) => {
  ...
  let qtyTotal = listTrans.reduce((cnt, o) => cnt + o.qtyOut, 0)
  ...
  const handleXLS = () => {
    if (fromDate === '' && toDate === '') {
      ...
      for (let n = 0; n < listTrans.length; n++) {
        ...
        sheet.getCell(`D${m}`).value = `${(parseFloat(listTrans[n].qtyOut)).toLocaleString(['ban', 'id'], {minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
