...
const Counter = ({ advertising, userStore, loading, dispatch, location, app }) => {
  ...
  const listProps = {
    ...
    storeInfo,
    pagination,
    loading: loading.effects['advertising/query'],
    location,
    onChange (page) {
    ...
    editItem (item) {
      ...
      dispatch({
        type: 'advertising/editItem',
        payload: { item }
      })
    },
    deleteItem (id) {
      dispatch({
        type: 'advertising/delete',
        payload: id
      })
    }
  }
  ...
  const changeTab = (key) => {
    ...
    dispatch({ type: 'advertising/updateState', payload: { list: [] } })
  }

  const clickBrowse = () => {
    dispatch({
      type: 'advertising/updateState',
      payload: {
        activeKey: '1'
      }
    })
